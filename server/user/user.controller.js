...
exports.getProfileUser = async (req, res) => {
  try {
    ...
    if (isFollowExist && isFollowExist._id) {
      isFollow = true;
    }
  ...
  try {
    ...
    if (setting.showFake) {
      users = await User.find({
        ...
        .populate("level")
        .select(
          "name username gender age image country bio followers following video post level isVIP loginType"
        );
    } else {
      users = await User.find({
        _id: { $ne: user._id },
      ...
      user:
        shuffleUser.length > 0
          ? {
              ...
              isFake: shuffleUser[0].loginType === 3 ? true : false,
            }
          : null,
    });
  } catch (error) {
