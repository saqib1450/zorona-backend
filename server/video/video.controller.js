...
exports.uploadVideo = async (req, res) => {
  try {
    ...
    video.thumbnail = req.files.thumbnail ? req.files.thumbnail[0].path : null;
...
exports.getVideo = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      message: video[0].video.length > 0 ? "Success!!" : "No Data Found !",
...
exports.getVideoById = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: video[0].video.length > 0 ? true : false,
      message:
        video[0].video.length > 0 ? "Success!!" : "Video does not Exist !",
      video: video[0].video.length > 0 ? video[0].video : [],
    });
  } catch (error) {
    console.log(error);
